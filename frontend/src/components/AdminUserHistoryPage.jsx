...
const AdminUserHistoryPage = () => {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const url = `${API_CONFIG.BASE_URL}${API_CONFIG.ADMIN.USER_HISTORY(userId)}`;
  ...
  useEffect(() => {
    if (token && userId) fetchHistory();
  }, [token, userId, fetchHistory]);

  const handleDownload = async (documentId) => {
    try {
      const url = `${API_CONFIG.BASE_URL}${API_CONFIG.ADMIN.PDF_DOWNLOAD(documentId)}`;
      const response = await axios.get(url, { responseType: "blob" });
      const disposition = response.headers["content-disposition"];
      const fileName = disposition
        ? disposition.split("filename=")[1]?.replace(/"/g, "").trim() || `document-${documentId}.pdf`
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{t("adminHistory.title")}</h1>
            <p className="text-gray-600 mt-1">{t("adminHistory.subtitle")}</p>
          </div>
        </div>
        <button
          type="button"
                  ...
                  <tr key={d.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-right">
                      ...
                      {d.documents?.length > 0 && (
                        <button
                          type="button"
                          onClick={() => handleDownload(d.documents[0].id)}
            ...
            <div className="relative bg-white rounded-lg shadow-xl max-w-2xl w-full p-6">
              ...
              <div className="mt-6 flex justify-end gap-2">
                {selectedDemande.documents?.length > 0 && (
                  <button
                    type="button"
                    onClick={() => handleDownload(selectedDemande.documents[0].id)}
